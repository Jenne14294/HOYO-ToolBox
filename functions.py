...
def get_average(file_path, game, input_text):
    ...
    if game == "原神":
        ...
        category_map["bangboo"] = {"type": "5", "list": bangboo, "fivestar_list": fivestar_bangboo}
    ...
    if file_path:
            ...
            for item in value:
                for category, details in category_map.items():
                    ...
                    if game != "絕區零":
                        ...
                        if item["gacha_type"] == category_type and category_type != "5":
        ...
        char_guarantee_rate = f"{round(((len(limit_char) / len(fivestar_char)) * 100), 2)} %" if len(fivestar_char) > 0 else None 
        weapon_guarantee_rate = f"{round(((len(limit_weapon) / len(fivestar_weapon)) * 100), 2)} %" if len(fivestar_weapon) > 0 else None
        ...
        status_text += f"武器池抽數：{len(weapons)} | 限定武器數：{len(limit_weapon)} | 平均限定金：{average_limit_weapon} | 限定五星數：{len(fivestar_weapon)} | 平均五星金：{average_weapon} | 保底不歪率：{weapon_guarantee_rate}\n"
        ...
        if game == "原神":
            ...
            coll_guarantee_rate = f"{round(((len(limit_coll) / len(fivestar_coll)) * 100), 2)} %" if len(fivestar_coll) > 0 else None 

            status_text += f"集錄池抽數：{len(collection)} | 限定角色數：{len(limit_coll)} | 平均限定金：{average_limit_collection} | 限定五星數：{len(fivestar_coll)} | 平均五星金：{average_collection} | 保底不歪率：{coll_guarantee_rate}\n"
